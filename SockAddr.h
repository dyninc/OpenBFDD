...
namespace openbfdd
{
  ...
  public:
     ...
     * If port is 0 then HasPort() will be false. 
     */
    explicit SockAddr(const IpAddr &src, in_port_t port = 0) : sockAddrBase(true,  (const sockAddrBase &)src) {if(port) SetPort(port);}
     ...
     */
    struct LessClass {bool operator() (const SockAddr& lhs, const SockAddr& rhs) const {return lhs.compare(rhs,  true) < 0;}};

    /**
     * Comparison class for STL containers, ignores port. 
     */
    struct LessNoPortClass {bool operator() (const SockAddr& lhs, const SockAddr& rhs) const {return lhs.compare(rhs,  false) < 0;}};
  };
  ...
  public:
     ...
     * Comparison class for STL containers. 
     */
    struct LessClass {bool operator() (const IpAddr& lhs, const IpAddr& rhs) const {return lhs.compare(rhs,  false) < 0;}};
